...
var primary1 =
    document.documentElement.style.getPropertyValue('--primary-1');  // #E85A4F
...
function sortBy(filter) {
  ...
  // only do stuff if the clicked button's content isn't currently being
  // displayed
  if (filteredItems.style.display != 'block') {
    // hide all currently displayed divs, then show the correct one + highlight
    // selected category
    ...
    document.getElementById('sortMechanicalBtn').style.backgroundColor =
        primary1;
    document.getElementById('sortCourseworkBtn').style.backgroundColor =
        primary1;
...
function darkMode() {
  ...
  // fade shadows out (ugly in dark mode)
  document.querySelector(':root').style.setProperty(
      '--box-shadow-color', 'rgba(0, 0, 0, 0.0)');

  document.documentElement.style.setProperty(
      '--bg-1-filter',
      'invert(23%) sepia(6%) saturate(147%) hue-rotate(169deg) brightness(93%) contrast(94%)');
}
...
function lightMode() {
  ...
  document.documentElement.style.setProperty(
      '--bg-1-filter',
      'invert(98%) sepia(32%) saturate(209%) hue-rotate(325deg) brightness(98%) contrast(86%)');
}
